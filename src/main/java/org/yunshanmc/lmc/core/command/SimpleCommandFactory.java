...
public final class SimpleCommandFactory {
        ...
        try {
            if (PlatformUtils.isBukkit()) {
                ...
                lmcSenderConverter = LOOKUP.unreflectConstructor(
                        LMCCommandSenderImpl.class
                                .getDeclaredConstructor(senderType));
            } else if (PlatformUtils.isBungeeCord()) {
                senderType = net.md_5.bungee.api.CommandSender.class;
                lmcSenderConverter = LOOKUP.unreflectConstructor(
                        org.yunshanmc.lmc.core.bungee.command.LMCCommandSenderImpl.class
                                .getDeclaredConstructor(senderType));
        ...
        return Arrays.stream(rawCmd.getClass().getDeclaredMethods()).filter(
            ...
            try {
                ...
                if (Strings.isNullOrEmpty(cmdAnno.usage())) {
                    ...
                    if (messageManager.getMessage(path) instanceof Message.MissingMessage) {
    ...
    static class SimpleCommandImpl extends LMCCommand {
        ...
        private SimpleCommandImpl(SimpleCommandFactory.SimpleCommandInfo cmdInfo, MessageSender messageSender, MethodHandle handle,
